use crate::executor::{run_ready_task_global_runtime, spawn, Executor};
...
/// Not unwind safe.
fn with_global_runtime_enabled<F, R>(f: F) -> R
where
    F: FnOnce() -> R,
{
    RUNTIME.with(|ref_cell| match ref_cell.replace(Some(Runtime::new())) {
        None => (),
        Some(_) => panic!("global runtime already enabled"),
    });
    let r = f();
    RUNTIME.with(|ref_cell| match ref_cell.replace(None) {
...
pub fn with_global_runtime_mut<F, R>(f: F) -> R
where
    F: FnOnce(&mut Runtime) -> R,
{
    RUNTIME.with(move |ref_cell| {
...
fn run_ready_tasks_batch_global_runtime(n: usize) -> bool {
    for _ in 0..n {
        if !run_ready_task_global_runtime() {
            return false;
        }
    }
    true
...
pub fn block_on<F: Future + 'static>(future: F) -> Result<F::Output, io::Error> {
    with_global_runtime_enabled(move || {
        ...
        loop {
            ...
            if more_tasks {
                ...
                with_global_runtime_mut(|rt| rt.reactor.poll())?;
            } else {
                let waken = with_global_runtime_mut(|rt| rt.reactor.wait())?;
                if waken == 0 {
